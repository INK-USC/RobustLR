...
class LRDiscriminative(BaseModel):
	def __init__(self, arch='roberta_large_race', train_batch_size=16, eval_batch_size=16, accumulate_grad_batches=1, learning_rate=1e-5, max_epochs=5,\
					optimizer='adamw', adam_epsilon=1e-8, weight_decay=0.0, lr_scheduler='linear_with_warmup', warmup_updates=0.0, gpus=1,\
					hf_name='LIAMF-USP/roberta-large-finetuned-race', save_dir=None, num_classes=3, compute_metrics=False):
	...
	def forward(self, batch):
		cls_emb   = self.text_encoder(input_ids=batch['all_sents'], attention_mask=batch['attn_mask'])['pooler_output']
	...
	def run_step(self, batch, split):
		...
		return {'loss': loss, 'preds': preds, 'targets': targets, 'neg_indicator': batch['has_neg'], 'iid': batch['iid'], 'theory_ids': batch['theory_ids']}
